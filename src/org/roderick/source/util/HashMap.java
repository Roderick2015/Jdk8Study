...
public class HashMap<K, V> extends AbstractMap<K, V> implements Map<K, V>, Cloneable, Serializable {
	...
	/**
	 * 这个方法是对键(key)的哈希值进行位运算，然后在put方法中通过(n - 1) & hash计算出散列在数组中的位置。
	 * 这里需要面对3个问题：
	 * 1.需在很短的时间(O(1))内计算出索引位置且必须在容器内，不能出现越界。
	 * 2.必须散列均匀，如果hash碰撞过多，那就成链表查询了。
	 * 3.当数据达到一定量时，必将会有更多的碰撞，那链表越来越长时，该如何处理？
	 * 
	 * 第一个问题我们可以使用key与table.length(容器数组的长度)作取模运算获得，HashMap也是这么干的，但是它干的更好。
	 * 我们需要知道当n(容器数组的长度)的长度是2的n次方时，(n - 1) & hash是等价于hash % n的，这也就是为什么HashMap会对你指定的初始容量进行修改,
	 * 而位于运算显然更优于取模运算，有兴趣的读者可以拿起笔去证实一下。
	 * 第二个问题HashMap是通过hash方法以及适当的扩容来实现优化的，该方法的重点是让高位也参与了计算，这样可以得到更多、更均匀的结果，比Java7中HashMap
	 * 的优化更进一步。
	 * 第三个问题在Java7中并没有优化处理，因此在碰撞较多的情况下，效率并不理想。Java8中则将超过一定长度的链表转为红黑树存储，时间复杂度由n减少到了log N。
	 * 因此碰撞越多优化效果越明显，但代码复杂度也是直线上升啊。
	 */
	static final int hash(Object key) {
        int h; 
        //无符号右移16位，即hash值本身的高位与低位的异或运算。
	...
	@SuppressWarnings({"rawtypes","unchecked"}) // for cast to Comparable5
	...
	/**
     * 根据你给定的容量大小，返回一个2的幂次方的值，
     * 也就是对你设定的值进行一个修整，至于干嘛这么做，我们会在hash方法中讲到。
     */
    ...
    /**
     * Node数组，hashmap存储映射数据的容器，后面简称容器
     */
    ...
    /**
     * 记录对HashMap结构性改变的次数，就是在增加、删除或者它自己重新hash扩容的时候modCount+1，
     * 所以对value的修改其实是不记录的。
     * 
    ...
    /**
     * 临界值 = table(容器)的大小 * loadFactor，当实际元素的个数(size)大于它的时候，HashMap就会开始扩容
     */
    int threshold;
    ...
    final V putVal(int hash, K key, V value, boolean onlyIfAbsent,
        ...
        //也只是暂时赋给了threshold，当然这个值是resize方法的根本依据。
        if ((tab = table) == null || (n = tab.length) == 0)
            n = (tab = resize()).length;
    ...
    final Node<K,V>[] resize() {
        ...
        if (oldCap > 0) {
            ...
            newThr = (int)(DEFAULT_LOAD_FACTOR * DEFAULT_INITIAL_CAPACITY); //0.75 * 16 = 12 首次扩容后的临界值
        }
        if (newThr == 0) {
    ...
    final Node<K,V> removeNode(int hash, Object key, Object value,
       ...
       //判断该索引位置上，有没有node
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            //matchValue如果传的是true，则通过后面条件判断，也就是value的值相等时，才会执行删除。
    ...
    abstract class HashIterator {
        ...
        final Node<K,V> nextNode() {
            ...
            if ((next = (current = e).next) == null && (t = table) != null) {
            	//返回next当前指向的Node，并移向HashMap的下一个Node。
                do {} while (index < t.length && (next = t[index++]) == null);
            }
            return e;
        }
